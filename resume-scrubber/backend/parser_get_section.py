...
#             "exchange programs",
#             "education and additional experiences",
...
#         r'\d{4}\s*;\s*\d+\s*\('       # e.g. "2024;42(4)"
#         r'|\d{4}\s*;\s*\d+\s*:'       # e.g. "2022; 28(12):1350"
#         r'|\b(?:vol|pp|pages?)\b'      # volume/page markers
#         r'|\bIn\s+press\b'            # "In press"
#         r'|\bet\s+al\b'               # "et al"
#         , re.IGNORECASE
#     )
#     _AUTHOR_LIST_RE = re.compile(
#         r'^[A-Z][a-z]+\s+[A-Z]{1,2},'  # "LastName AB," pattern
#         r'.*[A-Z][a-z]+\s+[A-Z]{1,2}'  # at least 2 such patterns
#     )

#     @staticmethod
#     def _looks_like_publication(line: str) -> bool:
#         """
#         Detect if a line looks like a publication/citation entry.
#         Used to split publications out of the experience section when
#         there's no explicit header between them.
#         """
#         stripped = line.strip()
#         if not stripped or len(stripped) < 40:
#             return False

#         # Strong signals: DOI or citation volume/page format
#         if SectionParser._DOI_RE.search(stripped):
#             return True
#         if SectionParser._CITATION_RE.search(stripped):
#             return True

#         # Author list pattern: "LastName AB, LastName CD, ..."
...
#         # Treat & as "and" before stripping special characters
...
#         When multiple keywords from different sections match with the same
#         word count, the keyword appearing later in the line wins (the
#         "head noun" heuristic — in English the main topic tends to be
#         rightmost, e.g. "Research Publications" → publications).
#         """
...
#         # Collect all matches: (kw_word_count, position_in_line, section)
#         matches = []
...
#                     break  # only first occurrence of this keyword

#         if not matches:
#             return None

#         # Pick the best match:
#         # 1. Longest keyword (most words) wins
#         # 2. Tie-break: keyword appearing later in the line wins
...
#                     if current_section == "experience" and SectionParser._looks_like_publication(line):
#                         SectionParser._save_section(sections, current_section, current_content)
